/* LinkedList.java
 *
 * This is a simple single linked list class which has a recursive
 * reverseToString method
 *  
...
public class LinkedList<R> implements Iterable<R>
{
	...
	/** Reverse of the toString
	  * Prints elements in a reversed order
	  * @return Returns string of elements in reversed order
	  */
	public String reverseToString()
	{
		/* Starting from index zero to let it travers from begining to end */
		return recursionHelper(ZERO);
	}
	
	
	
	/** Helper function
	  * Helps reverseToString for recursion
	  * @param cursor - The current index of the function
	  * @return If cursor points last element returns it else returns current
	     element after next one
	  */
	private String recursionHelper(int cursor)
	{
		if (cursor == size-ONE)
			return get(cursor).toString();
		
		else
			return (recursionHelper(cursor+ONE) + " <== " + get(cursor));
	}
