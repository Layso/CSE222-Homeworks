...
public class BinaryTree <R> implements Iterable<R>
{
	...
	public void add(R item)
	{
		...
		{
			AddToTree(root, level - 2, levelSize, item);
		}
	 ...
	 */
	private void AddToTree(Node<R> head, int step, int sizeLeft, R item)
	{
		...
		if (step == 0)
		{
			...
			if (sizeLeft%2 == 1)
			{
				head.right = new Node<R>(item);
			}

			/* To the left node */
			else
			{
				head.left = new Node<R>(item);
			}
